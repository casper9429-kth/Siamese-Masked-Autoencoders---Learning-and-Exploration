...
class FineTuneSiamMAE(nn.Module):
    """ 
        Light Weight Siamese Masked Autoencoder with VisionTransformer backbone, just the encoder part.
    """
    img_size : int = 224
    patch_size : int = 16
    in_chans : int = 3
    ...
    def __call__(self, frames1):
        """
            Forward pass through the whole network.
        """
        f1_embed = self.forward_encoder(frames1)
        return f1_embed
